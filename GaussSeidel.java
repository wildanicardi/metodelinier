...
public class GaussSeidel {
    ...
    //Menukar baris matrix jika nilai diagonalnya kurang dari kanan dan kirinya
    for (i = 0; i < n; i++) {
        for (j = 0; j < n + 1; j++) {
        if(i==0){
            B[i][j]=A[i][j];
    ...
    //Menukar Kolom 1 dengan kolom ke-3
    for (i = 0; i < n; i++) {
        for (j = 0; j < n + 1; j++) {
        if(j==0){
            B[i][j]=A[i][j];
    ...
    System.out.println("----------------------------------\n");
    System.out.println("Augmented Matriks setelah ditukar : ");
    //Mencetak matrix yang sudah ditukar
        for (i=0; i<n; i++){
            for (j=0; j<=n; j++){
                System.out.print(A[i][j] + "\t");
            }
